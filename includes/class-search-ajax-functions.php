...
if ( ! class_exists( 'Search_Ajax_Functions', false ) ) :
	...
	class Search_Ajax_Functions {
		 ...
		 * Callback function for handling the saving of the user's License Key.
		 */
		public function wpbooklist_search_save_options_action_callback() {

			global $wpdb;

			check_ajax_referer( 'wpbooklist_search_save_options_action_callback', 'security' );
			...
			if ( isset( $_POST['genre'] ) ) {
				$genre = filter_var( wp_unslash( $_POST['genre'] ), FILTER_SANITIZE_STRING );
			}
...
function wpbooklist_search_settings_action_javascript() { 
	?>
  	<script type="text/javascript" >
  	"use strict";
  	jQuery(document).ready(function($) {

  		$("#wpbooklist-search-img-remove-1").click(function(event){
  			$('#wpbooklist-search-preview-img-1').attr('src', '<?php echo ROOT_IMG_ICONS_URL ?>'+'book-placeholder.svg');
  		});

  		$("#wpbooklist-search-img-remove-2").click(function(event){
  			$('#wpbooklist-search-preview-img-2').attr('src', '<?php echo ROOT_IMG_ICONS_URL ?>'+'book-placeholder.svg');
  		});



	  	$("#wpbooklist-search-save-settings").click(function(event){

	  		$('#wpbooklist-search-success-div').html('');
	  		$('#wpbooklist-spinner-storfront-lib').animate({'opacity':'1'});

	  		var callToAction = $('#wpbooklist-search-call-to-action-input').val();
	  		var libImg = $('#wpbooklist-search-preview-img-1').attr('src');
	  		var bookImg = $('#wpbooklist-search-preview-img-2').attr('src');

		  	var data = {
				'action': 'wpbooklist_search_settings_action',
				'security': '<?php echo wp_create_nonce( "wpbooklist_search_settings_action_callback" ); ?>',
...
function wpbooklist_search_settings_action_callback(){
	global $wpdb;
	check_ajax_referer( 'wpbooklist_search_settings_action_callback', 'security' );
	...
	$table_name = SEARCH_PREFIX.'wpbooklist_jre_search_options';
...
function wpbooklist_search_save_default_action_javascript() { 

	$trans1 = __("Success!", 'wpbooklist');
	$trans2 = __("You've saved your default Search WooCommerce Settings!", 'wpbooklist');
	...
	?>
  	...
  	"use strict";
  	jQuery(document).ready(function($) {
	  	$("#wpbooklist-search-woo-settings-button").click(function(event){

	  		$('#wpbooklist-search-woo-set-success-div').html('');
			...
			// Making checks to see if Search extension is active
			...
			if(salebegin != undefined && saleend != undefined){
		  	...
		  	var data = {
				'action': 'wpbooklist_search_save_action_default',
				'security': '<?php echo wp_create_nonce( "wpbooklist_search_save_default_action_callback" ); ?>',
				'saleprice':salePrice,
...
function wpbooklist_search_save_default_action_callback(){
	global $wpdb;
	check_ajax_referer( 'wpbooklist_search_save_default_action_callback', 'security' );
	...
	$data = array(
 	...
 	$table = $wpdb->prefix."wpbooklist_jre_search_options";
...
function wpbooklist_search_upcross_pop_action_javascript() { 
	?>
  	<script type="text/javascript" >
  	"use strict";
  	jQuery(document).ready(function($) {
		  	...
		  	var data = {
				'action': 'wpbooklist_search_upcross_pop_action',
				'security': '<?php echo wp_create_nonce( "wpbooklist_search_upcross_pop_action_callback" ); ?>',
			};
...
// Callback function for creating backups
function wpbooklist_search_upcross_pop_action_callback(){
	global $wpdb;
	check_ajax_referer( 'wpbooklist_search_upcross_pop_action_callback', 'security' );
		
	// Get saved settings
    $settings_table = $wpdb->prefix."wpbooklist_jre_search_options";
...
add_action( 'admin_footer', 'wpbooklist_search_action_javascript' );
add_action( 'wp_ajax_wpbooklist_search_action', 'wpbooklist_search_action_callback' );
add_action( 'wp_ajax_nopriv_wpbooklist_search_action', 'wpbooklist_search_action_callback' );


function wpbooklist_search_action_javascript() { 
	?>
  	<script type="text/javascript" >
  	"use strict";
  	jQuery(document).ready(function($) {
	  	$("#wpbooklist-admin-addbook-button").click(function(event){
		  	...
		  	var data = {
				'action': 'wpbooklist_search_action',
				'security': '<?php echo wp_create_nonce( "wpbooklist_search_action_callback" ); ?>',
			};
			console.log(data);
...
// Callback function for creating backups
function wpbooklist_search_action_callback(){
	global $wpdb;
	check_ajax_referer( 'wpbooklist_search_action_callback', 'security' );
