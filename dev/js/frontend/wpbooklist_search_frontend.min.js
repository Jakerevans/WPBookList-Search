...
jQuery( document ).ready( function( $ ) {
	...
	function wpbooklistSearchRefiningSubmit(){
		...
		$( '#wpbooklist-search-searchterm-refined-form' ).on( 'submit', function() {
			...
			// How to filter/modify existing search results...
			var filterterms = [];
			var filtervalues = [];
			...
			// If the Libraries are empty, add in just the default Library.
			if ( 1 > searchin.length ) {
				searchin.push( wpbooklistSearchPhpVariables.SEARCH_SPECIAL_PREFIX + 'wpbooklist_jre_saved_book_log' );
			}
			...
			// Get parameters from the current URL and add them into the arrays
			params = getParams(window.location.href);
			...
			if ( '' !== params.searchby & null !== params.searchby && undefined !== params.searchby ) {
				if ( -1 < params.searchby.includes( ',' ) ) {
					...
					searchby = searchby.concat(tempsearchby).unique(); 
				} else {
					// If there's only one 'searchby' term set, and it's not already in our 'saerchby' array, add it.
					if ( -1 < searchby.indexOf( params.searchby ) ) {
						searchby.push( params.searchby );
					}
				}
			}

			// Adding in existing URL 'searchin' values...
			if ( '' !== params.searchin & null !== params.searchin && undefined !== params.searchin ) {
				if ( -1 < params.searchin.includes( ',' ) ) {
					tempsearchin = params.searchin.split( ',' );
					// Merges both arrays and gets unique items
					searchin = searchin.concat(tempsearchin).unique(); 
				} else {
					// If there's only one 'searchin' term set, and it's not already in our 'saerchin' array, add it.
					if ( -1 < searchin.indexOf( params.searchin ) ) {
						searchin.push( params.searchin );
					}
				}
			}
